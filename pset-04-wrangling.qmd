...
cases_raw <- api|> request() |> req_perform() |> resp_body_json(simplifyVector = TRUE)
```
...
```{r}
api <- "https://data.cdc.gov/resource/pwn4-m3yp.json"
cases_raw2 <- api|> request() |> req_url_query(`$limit` = 10000000000) |>req_perform() |> resp_body_json(simplifyVector = TRUE) # redo with limit
...
cases_pop_20_21 |> ggplot(aes(x=date,y=100000*cases/pop, col=state)) + 
  ...
  theme(legend.key.size = unit(0.5, "cm"), axis.text.x = element_text(size = 5, angle=45)) # fix spacing and label sizing
```
...
```{r}
Deaths <- deaths_url |> request() |> req_url_query(`$limit` = 10000000000) |> # increase limit to get full data
...
Deaths |> group_by(state) |> summarise(sumdeaths=sum(deaths)) |> # total deaths by state
  arrange(-sumdeaths) |> slice(seq(2,11)) |> # sort high to low, and get top 10 (not including "United States")
